-- Fix token type mismatch for public approval link RPC
-- approval_token is stored as text on tarefas, so this RPC must accept text.
DROP FUNCTION IF EXISTS public.get_mockups_by_approval_token(uuid);
