import { Calendar, Clock, LogOut, ChevronLeft, ChevronRight, Home, MessageSquare, Video, ListChecks, Wallet } from "lucide-react";
...
export const FuncionarioSidebarContent = ({ onNavigate, collapsed = false, onToggleCollapse }: SidebarContentProps) => {
    ...
    <TooltipProvider delayDuration={0}>
      <div className="flex flex-col h-full overflow-hidden">
          ...
          {collapsed && onToggleCollapse ? (
            <Tooltip>
              <TooltipTrigger asChild>
                <Button variant="ghost" size="icon" onClick={onToggleCollapse} className="text-sidebar-foreground hover:bg-sidebar-accent">
            ...
            <div className="flex flex-col items-center gap-1">
              <img src={noktaLogoDefault} alt="Logo" className="h-8 w-auto object-contain transition-all brightness-0 invert" />
              <span className="text-xs text-sidebar-foreground/60">Área do Funcionário</span>
            </div>
          )}
        </div>
        ...
        <nav className={cn(
          ...
          {funcionarioNavigation.map((item) => (
            <div key={item.name}>
              {collapsed ? (
                <Tooltip>
                  <TooltipTrigger asChild>
        ...
        {onToggleCollapse && !collapsed && (
          <div className="px-3 py-2">
            <Button variant="ghost" size="sm" onClick={onToggleCollapse} className="w-full justify-center gap-2 text-sidebar-foreground hover:bg-sidebar-accent">
        ...
        <div className={cn("border-t border-sidebar-border space-y-2", collapsed ? "p-2" : "p-4")}>
          {collapsed ? (
              ...
              <TooltipTrigger asChild>
                <div className="flex items-center justify-center py-2">
                  <div className="h-8 w-8 rounded-full bg-gradient-primary flex items-center justify-center text-primary-foreground text-sm font-medium cursor-default">
                    {user?.user_metadata?.full_name?.charAt(0).toUpperCase() || user?.email?.charAt(0).toUpperCase() || "F"}
                  </div>
                </div>
              </TooltipTrigger>
            ...
            <div className="flex items-center gap-3 px-3 py-2">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-sidebar-foreground/60 truncate">Funcionário</p>
              </div>
            </div>
          )}
          ...
          {collapsed ? (
            <Tooltip>
              <TooltipTrigger asChild>
                <Button variant="outline" size="icon" onClick={handleLogout} className="w-full border-sidebar-border hover:bg-sidebar-accent">
            ...
            <Button variant="outline" size="sm" onClick={handleLogout} className="w-full justify-start gap-2 border-sidebar-border hover:bg-sidebar-accent">
              <LogOut className="h-4 w-4" />
              Sair
            </Button>
          )}
...
export const FuncionarioSidebar = ({ collapsed = false, onToggleCollapse }: FuncionarioSidebarProps) => (
  <aside className={cn(
    ...
    <FuncionarioSidebarContent collapsed={collapsed} onToggleCollapse={onToggleCollapse} />
  </aside>
);
